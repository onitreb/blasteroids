(() => {
  ...
  (() => {
    ...
    function drawPolyline(ctx2, pts, x, y, angle, color, lineWidth = 2, fillColor = null) {
      ctx2.save();
      ctx2.translate(x, y);
      ctx2.rotate(angle);
      ctx2.beginPath();
    ...
    function drawElectricTether(ctx2, from, to, rgb, intensity, timeSec, seedBase) {
      ...
      ctx2.lineCap = "round";
      ctx2.strokeStyle = rgbToRgba(rgb, lerp(0.02, 0.22, intensity));
      ctx2.lineWidth = lerp(2, 7, intensity);
      ctx2.shadowColor = rgbToRgba(rgb, 0.95);
      ctx2.shadowBlur = lerp(4, 14, intensity);
      ctx2.beginPath();
      ...
      ctx2.stroke();
      ctx2.shadowBlur = 0;
      ctx2.setLineDash([8, 12]);
      ctx2.lineDashOffset = -timeSec * lerp(40, 110, intensity);
      ctx2.strokeStyle = rgbToRgba(rgb, lerp(0.05, 0.9, intensity));
      ctx2.lineWidth = lerp(1, 2.5, intensity);
      ctx2.stroke();
      ctx2.restore();
    }
    ...
    function createGame({ width, height }) {
      ...
      const state = {
        ...
        worldCells: {
          sizePx: 320,
          asteroidCounts: /* @__PURE__ */ new Map(),
      ...
      function render(ctx2) {
          ...
          for (let by = oy - tile; by < h + tile; by += tile) {
            for (let bx = ox - tile; bx < w + tile; bx += tile) {
              for (const s of layer.stars) {
                ...
                ctx2.fillStyle = `rgba(${s.color.r},${s.color.g},${s.color.b},${starAlpha.toFixed(3)})`;
                ctx2.beginPath();
                ctx2.arc(px, py, starRadius, 0, Math.PI * 2);
                ctx2.fill();
              }
            }
          }
        ...
        ctx2.save();
        const zoom = Math.max(0.1, state.camera.zoom || 1);
        ctx2.translate(w * 0.5, h * 0.5);
        ctx2.scale(zoom, zoom);
        ctx2.translate(-state.camera.x, -state.camera.y);
        ctx2.save();
        ctx2.strokeStyle = "rgba(255,255,255,0.26)";
        ctx2.lineWidth = 2;
        ctx2.strokeRect(-state.world.w / 2, -state.world.h / 2, state.world.w, state.world.h);
        ctx2.restore();
        if (state.mode === "playing") {
          const tier = currentShipTier();
          const fieldR = currentForceFieldRadius();
          ...
          ctx2.save();
          ctx2.strokeStyle = tier.ringColor;
          ctx2.lineWidth = 3;
          ctx2.beginPath();
          ctx2.arc(state.ship.pos.x, state.ship.pos.y, fieldR, 0, Math.PI * 2);
          ctx2.stroke();
          if (pulse > 0) {
            ctx2.strokeStyle = `rgba(255,255,255,${lerp(0, 0.85, pulse).toFixed(3)})`;
            ctx2.lineWidth = lerp(2, 6, pulse);
            ctx2.beginPath();
            ctx2.arc(state.ship.pos.x, state.ship.pos.y, fieldR, 0, Math.PI * 2);
            ctx2.stroke();
            ctx2.strokeStyle = `rgba(255,89,100,${lerp(0, 0.55, pulse).toFixed(3)})`;
            ctx2.lineWidth = lerp(1, 4, pulse);
            ctx2.beginPath();
            ctx2.arc(state.ship.pos.x, state.ship.pos.y, fieldR + lerp(0, 10, pulse), 0, Math.PI * 2);
            ctx2.stroke();
          }
          if (tierShift > 0) {
            ctx2.strokeStyle = `rgba(255,255,255,${(tierShift * 0.9).toFixed(3)})`;
            ctx2.lineWidth = lerp(2, 8, tierShift);
            ctx2.beginPath();
            ctx2.arc(state.ship.pos.x, state.ship.pos.y, fieldR + lerp(0, 34, 1 - tierShift), 0, Math.PI * 2);
            ctx2.stroke();
          }
          ctx2.restore();
          if (state.settings.showAttractRadius) {
            ctx2.save();
            ctx2.strokeStyle = "rgba(86,183,255,0.12)";
            ctx2.lineWidth = 2;
            ctx2.setLineDash([10, 10]);
            ctx2.lineDashOffset = 0;
            ctx2.beginPath();
            ctx2.arc(state.ship.pos.x, state.ship.pos.y, attractR, 0, Math.PI * 2);
            ctx2.stroke();
            ctx2.restore();
          }
        }
        for (const a of state.asteroids) {
          ...
          if (pullFx > 0.01) {
            ...
            drawElectricTether(ctx2, a.pos, ringPoint, tier.ringRgb, pullFx, state.time, seed);
            ctx2.save();
            ctx2.globalCompositeOperation = "lighter";
            ctx2.strokeStyle = rgbToRgba(tier.ringRgb, lerp(0.05, 0.35, pullFx));
            ctx2.lineWidth = lerp(2, 7, pullFx);
            ctx2.shadowColor = rgbToRgba(tier.ringRgb, 0.9);
            ctx2.shadowBlur = lerp(3, 14, pullFx);
            drawPolyline(ctx2, a.shape, a.pos.x, a.pos.y, a.rot, ctx2.strokeStyle, ctx2.lineWidth);
            ctx2.restore();
          }
          ...
          drawPolyline(ctx2, a.shape, a.pos.x, a.pos.y, a.rot, color, 2, "rgba(0,0,0,0.92)");
        }
        for (const g of state.gems) {
          ...
          ctx2.save();
          ctx2.translate(g.pos.x, g.pos.y);
          ctx2.globalCompositeOperation = "lighter";
          ctx2.fillStyle = `rgba(${rr},${gg},${bb},${(0.14 + pulseA * 0.18).toFixed(3)})`;
          ctx2.beginPath();
          ctx2.arc(0, 0, r * 2.8, 0, Math.PI * 2);
          ctx2.fill();
          ctx2.fillStyle = `rgba(${rr},${gg},${bb},${(0.1 + pulseA * 0.11).toFixed(3)})`;
          ctx2.beginPath();
          ctx2.arc(0, 0, r * 4.2, 0, Math.PI * 2);
          ctx2.fill();
          ctx2.globalCompositeOperation = "source-over";
          ctx2.fillStyle = `rgba(${rr},${gg},${bb},${(0.4 + pulseA * 0.58).toFixed(3)})`;
          if (g.kind === "diamond") {
            ctx2.rotate(g.spin);
            ctx2.beginPath();
            ctx2.moveTo(0, -r);
            ...
            ctx2.fill();
            ctx2.strokeStyle = `rgba(255,255,255,${(0.16 + pulseA * 0.2).toFixed(3)})`;
            ctx2.lineWidth = 1;
            ctx2.stroke();
          }
          ctx2.restore();
        ...
        if (state.saucer) {
          const s = state.saucer;
          ctx2.save();
          ctx2.translate(s.pos.x, s.pos.y);
          ctx2.globalCompositeOperation = "lighter";
          ctx2.fillStyle = "rgba(86,183,255,0.14)";
          ctx2.beginPath();
          ctx2.ellipse(0, 0, s.radius * 1.6, s.radius * 0.85, 0, 0, Math.PI * 2);
          ctx2.fill();
          ctx2.globalCompositeOperation = "source-over";
          ctx2.strokeStyle = "rgba(86,183,255,0.92)";
          ctx2.lineWidth = 2;
          ctx2.beginPath();
          ctx2.ellipse(0, 3, s.radius * 1.25, s.radius * 0.45, 0, 0, Math.PI * 2);
          ctx2.stroke();
          ctx2.beginPath();
          ctx2.ellipse(0, -3, s.radius * 0.55, s.radius * 0.42, 0, Math.PI, 0);
          ctx2.stroke();
          ctx2.fillStyle = "rgba(231,240,255,0.78)";
          const winY = 2;
          for (let i = -1; i <= 1; i++) {
            ctx2.beginPath();
            ctx2.arc(i * (s.radius * 0.42), winY, 2.1, 0, Math.PI * 2);
            ctx2.fill();
          }
          ctx2.restore();
        }
        for (const b of state.saucerLasers) {
          ...
          const lenPx = 24;
          ctx2.save();
          ctx2.translate(b.pos.x, b.pos.y);
          ctx2.rotate(ang);
          ctx2.lineCap = "round";
          ctx2.globalCompositeOperation = "lighter";
          ctx2.strokeStyle = `rgba(255,221,88,${(alpha * 0.6).toFixed(3)})`;
          ctx2.lineWidth = 9;
          ctx2.shadowColor = "rgba(255,221,88,0.95)";
          ctx2.shadowBlur = 12;
          ctx2.beginPath();
          ctx2.moveTo(-lenPx * 0.5, 0);
          ctx2.lineTo(lenPx * 0.5, 0);
          ctx2.stroke();
          ctx2.globalCompositeOperation = "source-over";
          ctx2.shadowBlur = 0;
          ctx2.strokeStyle = `rgba(255,250,190,${alpha.toFixed(3)})`;
          ctx2.lineWidth = 3;
          ctx2.beginPath();
          ctx2.moveTo(-lenPx * 0.5, 0);
          ctx2.lineTo(lenPx * 0.5, 0);
        ...
        for (const e of state.effects) {
          ...
          ctx2.strokeStyle = `rgb(${rr},${gg},${bb})`;
          ctx2.lineWidth = e.kind === "ring" ? lerp(6, 1, t) : lerp(3, 1, t);
          ctx2.beginPath();
          ctx2.arc(0, 0, r, 0, Math.PI * 2);
          ctx2.stroke();
          if (e.kind === "ring" || e.kind === "pop") {
            ...
            for (let i = 0; i < rays; i++) {
              ...
              ctx2.beginPath();
              ctx2.moveTo(Math.cos(ang) * r0, Math.sin(ang) * r0);
              ctx2.lineTo(Math.cos(ang) * r1, Math.sin(ang) * r1);
              ctx2.stroke();
            }
          }
          ctx2.restore();
        }
        drawShipModel(ctx2, state.ship, state.mode === "playing" && state.input.up);
        ctx2.restore();
        ctx2.save();
        ctx2.fillStyle = "rgba(231,240,255,0.85)";
        ...
        if (state.mode === "playing") {
          ...
          ctx2.fillStyle = "rgba(255,89,100,0.92)";
          ctx2.font = "16px ui-sans-serif, system-ui";
          ctx2.fillText("Impact with a heavy asteroid. Press R to restart.", 14, 28);
        }
        ctx2.restore();
      }
