...
| MP-25 | M3 | Visual parity: client-side VFX while MP connected | RENDER/NET | NOT_STARTED | MP-24 | manual | Restore visual-only effects in MP by deriving VFX from authoritative state changes + local input (no gameplay side effects): thrusters/exhaust, pull/burst FX, and round-loop “sonar”/tech ping + part visibility cues. |
