...
| MP-22 | M1 | Client prediction: local ship turn/thrust immediate + reconcile | NET/ENGINE (Codex) | DONE | MP-18 | manual | Notes (standards + refs):<br>- Colyseus does **not** provide built-in client prediction yet; implement app-side (FAQ section “Does Colyseus help me with client-prediction?”): https://docs.colyseus.io/faq<br>- Official references for predicted input + fixed tick (v0.17 Phaser tutorial):<br>  - https://docs.colyseus.io/tutorial/phaser/client-predicted-input (shows `server.simulateLatency(200)` for testing)<br>  - https://docs.colyseus.io/tutorial/phaser/fixed-tickrate<br>- Command Pattern best practice (queue/process commands deterministically): https://docs.colyseus.io/recommendations/command-pattern<br><br>Plan (implementation, mapped to repo files):<br>- Predict **local ship only** (turn/thrust). Remote ships + asteroids + gems stay server-authoritative and are rendered from interpolated snapshots (`src/net/createMpWorldView.js`).<br>- Client input commands: add `seq` to input payloads, send at 60Hz (fixed tick), and keep a ring buffer of unacked commands for replay (`src/net/createMpClient.js`).<br>- Authoritative ack: extend Schema `PlayerState` with `lastProcessedInputSeq` so the client can drop acknowledged inputs and reconcile (`server/schema/BlasteroidsState.mjs`).<br>- Server input queue: queue incoming input messages per client, apply in-order at the start of each 60Hz sim tick (ignore stale/out-of-order), set `lastProcessedInputSeq`, then run one authoritative engine tick (`server/rooms/BlasteroidsRoom.mjs`).<br>- Client reconciliation: on patch, read the authoritative local-player pose + `lastProcessedInputSeq`, rollback local predicted ship to server pose, replay remaining unacked commands using the same deterministic ship-step, then render predicted local ship + interpolated world (`src/net/createMpWorldView.js` + `src/main.js`).<br>- Drift minimization: extract ship kinematics into a deterministic helper used by both authoritative engine update and client prediction (DOM-free, no RNG, no gameplay side effects) so movement math matches (`src/engine/*`).<br>- Ensure `createMpWorldView.applyInterpolatedState` does **not** overwrite predicted local ship fields while prediction is enabled (keep authoritative local pose for reconciliation/debug).<br><br>Validation (reality-based):<br>- Manual: enable simulated latency (Colyseus `simulateLatency`) and confirm local ship responds immediately while server remains authoritative; observe bounded correction (no runaway rubber-banding).<br>- Automated: extend Playwright MP smoke to run with simulated latency + lower patch Hz, then assert local movement begins before the first server patch; keep `file://` singleplayer unchanged. |
| MP-23 | M3 | Gameplay parity: enable + sync round loop (star/gate/parts) in MP | ENGINE/SERVER/NET/RENDER (Codex) | DONE | MP-21 | manual + smokes | Notes:<br>- Server: MP rooms enable `features.roundLoop` and auto-reset the round after win/lose so the room stays playable.<br>- Server exports round loop state into Schema: star + gate + tech parts (including per-player carried parts).<br>- Client: MP world view ingests + applies round state so existing renderer draws the round loop (star, gate, dropped/carried parts).<br>- Engine: round loop refactored to be multi-ship aware (per-player star exposure, per-player part carry/pickup/install); preserves singleplayer escape animation locally while server uses a simple “any ship enters active gate => win”.<br>- Validation: `npm test` (pass); `npm run build` (pass; updated `dist/blasteroids.js`); `node scripts/mp-browser-smoke.mjs` (pass); `node scripts/mp-browser-2p-smoke.mjs` (pass); `node scripts/mp-lan-smoke.mjs ws://localhost:2567` (pass). |
| MP-24 | M3 | Gameplay parity: enable + sync saucer + lasers in MP | ENGINE/SERVER/NET/RENDER (Codex) | DONE | MP-23 | manual + smokes | Notes:<br>- Server: enable `features.saucer` for MP rooms; sync saucer + lasers into Schema.<br>- Engine: saucer targeting + laser collisions are multi-ship aware (deterministic tie-break by player id).<br>- Client: MP world view ingests/interpolates saucer + lasers so existing renderer draws them; singleplayer path unchanged. |
| MP-25 | M3 | Visual parity: client-side VFX while MP connected | RENDER/NET (Codex) | DONE | MP-24 | manual | Notes:<br>- Client-only MP VFX updater runs while MP is connected (no gameplay side effects): exhaust particles + burst wavelets + tech ping (sonar) with cooldown + highlights.<br>- Renderer now shows remote ship thrusters using a cheap heuristic derived from authoritative motion (no net sync). |
| MP-27 | M3 | Visual parity: pull/gravity + burst/blast FX in MP | RENDER/NET (Codex) | DONE | MP-25 | manual + smokes | Notes:<br>- “Gravity”/pull tethers now render in MP by deriving intensity from authoritative `pullOwnerId` + distance (client-only; no gameplay impact).<br>- Burst/blast feedback now renders in MP for local + remote players by detecting authoritative burst state changes (asteroids detaching + `shipLaunched`), driving `blastPulseT` client-side, and spawning wavelets/ring effects (no gameplay impact). |
| MP-28 | M3 | Visual parity: impact/pickup FX + ambient animations in MP | RENDER/NET (Codex) | DONE | MP-27 | manual + smokes | Notes:<br>- MP-connected client-only VFX derived from authoritative state changes: gem pickup sparkle, tech-part pickup/install glint, ship-launched projectile impact pops (heuristic, view-gated), saucer kill pop/ring (gold-gem heuristic).<br>- Ambient animation restored while MP connected: gem spin/pulse and star-heat glows for ships/asteroids derived from round star boundary (no gameplay impact). |
...
### 2026-02-20 (MP-23 kickoff)
- Marked MP-23 `IN_PROGRESS` with an execution plan focused on server-authoritative round loop + client rendering parity.

### 2026-02-20 (MP-23)
- Implemented round loop parity in LAN multiplayer:
  - Server enables `features.roundLoop`, syncs round state via Schema, and auto-resets rounds after win/lose.
  - Engine round loop is multi-ship aware for part carry/pickup/install and star exposure.
  - Client MP view applies round loop state so renderer shows star/gate/parts while connected.
- Validation: `npm test` (pass); `npm run build` (pass; updated `dist/blasteroids.js`); `node scripts/mp-browser-smoke.mjs` (pass); `node scripts/mp-browser-2p-smoke.mjs` (pass); `node scripts/mp-lan-smoke.mjs ws://localhost:2567` (pass).

### 2026-02-20 (MP-23 follow-ups)
- Fixed Colyseus Schema runtime error: avoid growing `ArraySchema.length` directly; resize via `push/pop` then assign indices.
- Reduced visible MP “spawn pop-in”: server now excludes client-provided view rects from ambient spawn placement and skips the singleplayer-only “guarantee onscreen asteroids” logic.

### 2026-02-20 (MP-24)
- Enabled saucer + lasers in multiplayer with server-authoritative behavior and client rendering:
  - Server: MP rooms now run with `features.saucer: true` and export saucer + laser state into Schema.
  - Engine: saucer targets the nearest ship (deterministic tie-break by player id) and saucer laser collisions test all ships.
  - Client: MP world view ingests/interpolates saucer + lasers into engine state so the existing renderer draws them.
- Perf follow-up: cap concurrent saucer lasers (`state.params.saucerLaserMaxCount`, default `16`) and render lasers in a cheaper mode while MP-connected; MP HUD now shows laser count (`... ${lasers}l ...`).
- Validation: `npm test` (pass); `npm run build` (pass; updated `dist/blasteroids.js`); `node scripts/mp-browser-smoke.mjs` (pass); `node scripts/mp-browser-2p-smoke.mjs` (pass); `node scripts/mp-browser-saucer-smoke.mjs` (pass); `node scripts/mp-lan-smoke.mjs ws://localhost:<port>` (pass).

### 2026-02-20 (MP-25)
- Restored MP-connected visual parity for client-only effects (no gameplay side effects):
  - Added MP VFX updater that runs while MP is connected: exhaust particles (all ships), burst wavelets (local input), and tech ping/sonar ring with cooldown + highlights for parts/part asteroids.
  - Renderer shows remote ship thrusters via a cheap heuristic derived from authoritative motion (no net sync).
- Validation: `npm test` (pass); `npm run build` (pass; updated `dist/blasteroids.js`); `node scripts/mp-browser-smoke.mjs` (pass); `node scripts/mp-browser-2p-smoke.mjs` (pass); manual LAN play (verify thrusters/exhaust + burst wavelets + sonar ring; no FPS regression).

### 2026-02-20 (MP-27)
- Restored additional MP-connected visual parity items:
  - “Gravity”/pull tethers now render in MP by deriving pull intensity from authoritative `pullOwnerId` + distance (client-only).
  - Burst/blast feedback now renders in MP for local + remote players by detecting authoritative burst state changes (detaching asteroids + `shipLaunched`) and driving ring pulse (`blastPulseT`) + wavelets/ring effects client-side.
- Validation: `npm test` (pass); `npm run build` (pass; updated `dist/blasteroids.js`); `node scripts/mp-browser-smoke.mjs` (pass); `node scripts/mp-browser-2p-smoke.mjs` (pass); manual LAN play (verify pull tethers + blast pulses + wavelets; no FPS regression).

### 2026-02-20 (MP-28)
- Restored MP-connected ambient animation + pickup/impact FX parity (client-only, derived from authoritative state):
  - Gem visuals now animate (spin + throb) while MP-connected, matching the singleplayer “throb” feel.
  - Star heat glows for ships + asteroids are derived from the authoritative star boundary so the red-giant pressure reads in MP.
  - Added view-gated impact/pickup FX: gem pickup sparkle, tech-part carry/install glint, ship-launched projectile impact pops, and saucer kill pop/ring (gold-gem heuristic).
- Validation: `npm test` (pass); `npm run build` (pass; updated `dist/blasteroids.js`); `node scripts/mp-browser-smoke.mjs` (pass); `node scripts/mp-browser-2p-smoke.mjs` (pass); manual LAN play (verify gem animation + star heat + pickup/impact pops; no FPS regression).

### 2026-02-22 (MP-22)
- Implemented local ship prediction + server-authoritative reconcile:
  - Server: input messages now carry a monotonic `seq`; the room queues input per client and applies in-order at the start of each 60Hz tick, syncing `PlayerState.lastProcessedInputSeq` as an ack.
  - Client: while MP-connected, the local ship is predicted using the same deterministic ship-step as the authoritative engine; on patch, the client rolls back to the latest server pose and replays any unacked inputs.
