...
| MP-20 | M1 | Interest management: per-client asteroid/gem visibility (in-view + margin) | SERVER/NET | NOT_STARTED | MP-11 | manual (2p big arena) | Prefer Colyseus `StateView` first; note docs warn `StateView` isn’t optimized for big datasets. If patch CPU/bytes still dominate, switch asteroids/gems to custom snapshot messages (Schema for players only). |
| MP-21 | M1 | Server sim scaling: sleep far asteroids (simulate only near players) | ENGINE/SERVER | NOT_STARTED | MP-20 | manual (stress) | Define “active region” as union of each player view + margin; only active asteroids do collisions/attachments. Sleeping asteroids drift analytically or at reduced tick; wake on proximity. Must remain deterministic. |
